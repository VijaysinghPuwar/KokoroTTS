...
import os, sys, re, time, threading, tempfile, traceback, datetime, subprocess, platform
...
VOICE_CANDIDATES = [
    ...
    ("Male — Leo (US)",       "am_leo"),
    # Optional UK (if your pack supports)
...
def get_diagnostics_text() -> str:
    ...
    if _is_apple_silicon():
        ...
        if metal: lines.append(f"Metal GPU: {metal}")
    if _is_windows():
        cuda_name = _get_gpu_name_cuda()
        if cuda_name: lines.append(f"CUDA GPU: {cuda_name}")
...
class KokoroApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.text_box = tk.Text(text_wrap, wrap="word", font=text_font, undo=True)
        ...
        # macOS uses Control-Click; Windows uses right-click
        ...
        # Compute list is dynamic based on available providers
        self.compute_combo = ttk.Combobox(controls, state="readonly", values=["AUTO", "CPU"])
